...
cache_or_compute <- function(path, compute_fun) {
  if (file.exists(path)) {
    readRDS(path)
  } else {
    value <- compute_fun()
...
The kernel-smoothed intensity surface highlights where monitors are more densely placed—a global first-order signal. We estimate the smoothing scale with the PPL likelihood cross-validation rule to avoid the overly narrow default bandwidth.

```{r}
library(ggplot2)
dir.create("plots", showWarnings = FALSE)

kernel_bw <- bw.ppl(monitor_ppp, srange = c(500, 60000)) * 4
kernel_intensity <- density(monitor_ppp, sigma = kernel_bw, edge = TRUE, dimyx = c(256, 256))
...
kernel_intensity_plot <- ggplot(kernel_df, aes(x = x, y = y, fill = value)) +
  ...
  labs(
    ...
    subtitle = sprintf("PPL bandwidth σ = %.0f m", kernel_bw),
...
```{r}
distance_surface <- as.im(dist_to_road_fun, W = la_window, dimyx = c(128, 128))
inhom_model <- ppm(
...
trend_plot <- ggplot(trend_df, aes(x = x, y = y, fill = value)) +
  ...
  labs(
    ...
    subtitle = "Poisson model with distance-to-road covariate (log scale)",
    x = "Easting (m)",
    y = "Northing (m)"
  ) +
  theme_minimal()
...
```{r}
set.seed(6805)
quadrat_fit <- quadrat.test(monitor_ppp, fit = inhom_model, nsim = 999)
quadrat_fit
```

## Second-order Structure and Monte Carlo Testing
We test whether monitor locations exhibit Complete Spatial Randomness (CSR) using an L-function envelope test with 999 Monte Carlo simulations. Under CSR, monitors would be randomly distributed across the study area with no spatial clustering or regularity. Significant departures from the CSR envelope indicate systematic spatial structure requiring explanation through covariates such as road proximity.
...
pcf_env_csr <- cache_or_compute(
  file.path(cache_dir, "pcf_env_csr.rds"),
  function() {
    set.seed(6805)
    envelope(
      monitor_ppp,
...
pcf_csr_plot <- ggplot(pcf_env_df, aes(x = r)) +
  ...
  labs(
    ...
    subtitle = "Observed g(r) compared with 999 CSR simulations",
    x = "r (meters)",
    y = "g(r)"
  ) +
  theme_minimal()
...
L_env <- cache_or_compute(
  file.path(cache_dir, "L_env_csr.rds"),
  function() {
    set.seed(6805)
    envelope(
      monitor_ppp,
...
L_plot <- ggplot(L_df, aes(x = r)) +
  ...
  geom_line(aes(y = L_centered), color = "#0072B2", linewidth = 1) +
  geom_hline(yintercept = 0, linetype = "dashed", color = "gray40") +
  labs(
    title = "L-function Envelope (CSR)",
    subtitle = "Observed L(r) - r compared with 999 CSR simulations",
    x = "r (meters)",
    y = "L(r) - r"
  ) +
  theme_minimal()
